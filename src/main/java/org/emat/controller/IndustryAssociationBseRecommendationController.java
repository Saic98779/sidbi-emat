...
public class IndustryAssociationBseRecommendationController {
    ...
    public ResponseEntity<ApiResponse<BseRecommendationResponse>> createBseRecommendation(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(
                        ApiResponse.created(
                                "BSE recommendation created successfully",
                                bseRecommendationService.createBseRecommendation(request)));
    }

    @GetMapping
    @PreAuthorize("hasAnyRole(@endpointRolePolicyService.resolveRoles('bseRecommendationRead'))")
    @Operation(
            summary = "Get all BSE recommendations",
            description = "Retrieve all active BSE recommendations")
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>> getAllBseRecommendations() {
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.getAllBseRecommendations()));
    }
    ...
    public ResponseEntity<ApiResponse<BseRecommendationResponse>> getBseRecommendationById(
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendation fetched successfully",
                        bseRecommendationService.getBseRecommendationById(id)));
    }
    ...
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>>
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.getBseRecommendationsByRegistration(
                                registrationId)));
    }
    ...
    public ResponseEntity<ApiResponse<BseRecommendationResponse>> updateBseRecommendation(
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendation updated successfully",
                        bseRecommendationService.updateBseRecommendation(id, request)));
    }
    ...
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>> searchByBseName(
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.searchByBseName(bseName)));
    }

    @GetMapping("/gt-recommendation")
    @PreAuthorize("hasAnyRole(@endpointRolePolicyService.resolveRoles('bseRecommendationRead'))")
    @Operation(
            summary = "Get BSE recommendations by GT status",
            description =
                    "Filter BSE recommendations where GT recommendation is not null or null based"
                            + " on isRecommended")
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>> getByGtRecommendation(
            @RequestParam boolean isRecommended) {
        log.info("REST request to get BSE recommendations with GT recommendation set");
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.getByGtRecommendation(isRecommended)));
    }

    @GetMapping("/pmu-recommendation")
    @PreAuthorize("hasAnyRole(@endpointRolePolicyService.resolveRoles('bseRecommendationRead'))")
    @Operation(
            summary = "Get BSE recommendations by PMU status",
            description =
                    "Filter BSE recommendations where PMU recommendation is not null or null based"
                            + " on isRecommended")
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>> getByPmuRecommendation(
            @RequestParam boolean isRecommended) {
        log.info("REST request to get BSE recommendations with PMU recommendation set");
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.getByPmuRecommendation(isRecommended)));
    }

    @GetMapping("/ho-recommendation")
    @PreAuthorize("hasAnyRole(@endpointRolePolicyService.resolveRoles('bseRecommendationRead'))")
    @Operation(
            summary = "Get BSE recommendations by HO status",
            description = "Filter BSE recommendations where HO recommendation is not null")
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>> getByHoRecommendation(
            @RequestParam boolean isRecommended) {
        log.info("REST request to get BSE recommendations with HO recommendation set");
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.getByHoRecommendation(isRecommended)));
    }

    @GetMapping("/mapped/{status}")
    @PreAuthorize("hasAnyRole(@endpointRolePolicyService.resolveRoles('bseRecommendationRead'))")
    @Operation(
            summary = "Get BSE recommendations by mapped status",
            description = "Filter BSE recommendations by IA mapping status")
    public ResponseEntity<ApiResponse<List<BseRecommendationResponse>>> getByMappedStatus(
            @PathVariable Boolean status) {
        log.info("REST request to get BSE recommendations by mapped status: {}", status);
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE recommendations fetched successfully",
                        bseRecommendationService.getByMappedStatus(status)));
    }

    @Operation(
            summary = "Get selected BSE recommendations by vendor",
            description =
                    "Fetches all active BSE recommendations mapped to the specified vendor where"
                            + " the recommendation is marked as selected.")
