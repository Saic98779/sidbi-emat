...
public class BseSalaryController {
    ...
    public ResponseEntity<ApiResponse<BseSalaryResponse>> create(
            @RequestBody BseSalaryRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(
                        ApiResponse.created(
                                "BSE salary created successfully",
                                bseSalaryService.create(request)));
    }
    ...
    public ResponseEntity<ApiResponse<BseSalaryResponse>> getById(@PathVariable Long id) {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE salary fetched successfully", bseSalaryService.getById(id)));
    }
    ...
    public ResponseEntity<ApiResponse<List<BseSalaryResponse>>> getAll() {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE salaries fetched successfully", bseSalaryService.getAll()));
    }

    @GetMapping("/approved-industry-associations")
    @PreAuthorize(
            "hasAnyRole(@endpointRolePolicyService.resolveRoles('bseSalaryApprovedIndustryAssociationsRead'))")
    public ResponseEntity<ApiResponse<List<String>>> getApprovedIndustryAssociationNames() {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Approved industry associations fetched successfully",
                        bseSalaryService.getApprovedIndustryAssociationNames()));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole(@endpointRolePolicyService.resolveRoles('bseSalaryUpdate'))")
    public ResponseEntity<ApiResponse<BseSalaryResponse>> update(
            @PathVariable Long id, @RequestBody BseSalaryUpdateRequest request) {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "BSE salary updated successfully", bseSalaryService.update(id, request)));
    }
