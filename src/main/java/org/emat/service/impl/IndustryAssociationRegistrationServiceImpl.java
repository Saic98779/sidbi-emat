...
@Transactional
public class IndustryAssociationRegistrationServiceImpl
        implements IndustryAssociationRegistrationService {
    ...
    private static final String IN_PRINCIPLE_APPROVAL_OF_IA_SUBMISSION =
            "IN_PRINCIPLE_APPROVAL_OF_IA_SUBMISSION";
    private static final String IN_PRINCIPLE_APPROVAL_OF_SDE_APPROVAL =
    ...
    public IndustryAssociationRegistrationResponse createRegistration(
        ...
        log.info(
                "Creating new Industry Association Registration for: {}",
        ...
        User sidbiApprover =
                commonUtil.resolveSidbiApprover(
                        request.getSidbeApprovedByUserId(), isSidbiSdeCaller);

        IndustryAssociationRegistration registration =
                registrationMapper.toEntity(request, isSidbiSdeCaller, sidbiApprover);

        IndustryAssociationRegistration saved = repository.save(registration);
        log.info(
                "Industry Association Registration created successfully with ID: {}",
                saved.getId());
    ...
    public IndustryAssociationRegistrationResponse approveBySidbe(
        ...
        log.info(
                "Processing SIDBE approval for registration with ID: {} by user: {}", id, username);
        IndustryAssociationRegistration registration = getRegistrationOrThrow(id);
        ...
        IndustryAssociationRegistration updated = repository.save(registration);
        log.info(
                "SIDBE approval processed successfully for registration ID: {} by user: {}",
                id,
                username);
