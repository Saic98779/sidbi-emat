...
@Slf4j
public class IndustryAssociationBseRecommendationServiceImpl
        implements IndustryAssociationBseRecommendationService {
    ...
    @Override
    @Transactional
    public BseRecommendationResponse createBseRecommendation(
        ...
        IndustryAssociationBseRecommendation bseRecommendation =
                bseRecommendationMapper.toEntity(
                        request, registration, user, commonUtil.getCurrentUsername());

        IndustryAssociationBseRecommendation saved =
                bseRecommendationRepository.save(bseRecommendation);
    ...
    public List<BseRecommendationResponse> getAllBseRecommendations() {
        ...
        return bseRecommendationRepository.findByIsActiveTrue().stream()
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BseRecommendationResponse> getBseRecommendationsByRegistration(
            Long registrationId) {
        log.info("Fetching BSE recommendations for registration: {}", registrationId);
        return bseRecommendationRepository.findByRegistrationId(registrationId).stream()
                .map(bseRecommendationMapper::toResponse)
                .toList();
    }
    ...
    @Override
    @Transactional
    public BseRecommendationResponse updateBseRecommendation(
        ...
        bseRecommendationMapper.applyUpdateRequest(
                request, bseRecommendation, user, commonUtil.getCurrentUsername());

        IndustryAssociationBseRecommendation updated =
                bseRecommendationRepository.save(bseRecommendation);
    ...
    public List<BseRecommendationResponse> searchByBseName(String bseName) {
        ...
        return bseRecommendationRepository
                .findByBseNameContainingIgnoreCaseAndIsActiveTrue(bseName)
                .stream()
                .map(bseRecommendationMapper::toResponse)
                .toList();
    }
    ...
    public List<BseRecommendationResponse> getByGtRecommendation(boolean isRecommended) {
        List<IndustryAssociationBseRecommendation> list =
                isRecommended
                        ? bseRecommendationRepository
                                .findByGtRecommendationIsNotNullAndIsActiveTrue()
    ...
    public List<BseRecommendationResponse> getByPmuRecommendation(boolean isRecommended) {
        List<IndustryAssociationBseRecommendation> list =
                isRecommended
                        ? bseRecommendationRepository
                                .findByPmuRecommendationIsNotNullAndIsActiveTrue()
                        : bseRecommendationRepository
                                .findByPmuRecommendationIsNullAndIsActiveTrue();
        return list.stream().map(bseRecommendationMapper::toResponse).toList();
    }
    ...
    public List<BseRecommendationResponse> getByHoRecommendation(boolean isRecommended) {
        List<IndustryAssociationBseRecommendation> list =
                isRecommended
                        ? bseRecommendationRepository
                                .findByHoRecommendationIsNotNullAndIsActiveTrue()
    ...
    public List<BseRecommendationResponse> getByMappedStatus(Boolean iaMapped) {
        return bseRecommendationRepository.findByIaMappedAndIsActiveTrue(iaMapped).stream()
                .map(bseRecommendationMapper::toResponse)
                .toList();
    }
